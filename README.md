...
* **Java 21**, **Node.js (npm)**, **Python 3.10+** 설치
* **MySQL**, **Redis** 서버 설치 및 실행

### 2. 환경 변수 설정 (Environment Variables)

* **Backend**: `src/main/resources/` 경로에 `application.yml` 파일을 생성하고 DB, Redis, JWT, API 키 등의 설정을 완료합니다. (`.gitignore`에 등록되어 있어 직접 생성해야 합니다.)
* **AI Server**: `ai-server/` 경로에 `.env` 파일을 생성하고 `ANTHROPIC_API_KEY` 등 필요한 API 키를 설정합니다.

### 3. 애플리케이션 실행 (Running the Apps)

각 애플리케이션을 실행하려면 3개의 터미널이 필요합니다.
...
# 3. 서버 실행 (http://localhost:8000)
python main.py
```
> AI 서버 실행 후 `http://localhost:8000/docs`에서 API 명세(Swagger)를 확인할 수 있습니다.
